...
int main(int argc, char **argv)
{
    ...
    while (ros::ok())
    {
        ...
        if (newJoyData && joyData.buttons[5] == 1)
        {
            ...
            double timeOut = 0;
            //Create csv files: 1.) with complete Data  2.) only Abort commands are saved
            ...
            //Variables for converting Quaternions to Euler Angles
            ...
            while (ros::ok() && joyData.buttons[4] == 0)
            {
                ...
                if ((ros::Time::now().toSec() - timeOut) > 3)
                    timeOut = 0;
                if (stopped && !wasStopped && timeOut == 0 && (lastYaw == 0 || (signbit(faces.facegeos[0].yaw) == !signbit(lastYaw))))
                {
                    numbStops++;
                    lastYaw = faces.facegeos[0].yaw;
